...
with DAG(
    ...
    task_sort = BashOperator(
        ...
        bash_command="""
            ...
            cat ~/data/cut/{{ds_nodash}}/cut.log | sort > ~/data/sort/{{ds_nodash}}/sort.log # sort 수행 결과  파일 저장 
        """
     )
    task_count = BashOperator(
        ...
        bash_command="""
            echo "count"
            mkdir -p ~/data/count/{{ds_nodash}} # 반복
            cat ~/data/sort/{{ds_nodash}}/sort.log | uniq -c > ~/data/count/{{ds_nodash}}/count.log # cat ~~~~~~~~~~~~~~/count.log
        """
     )
